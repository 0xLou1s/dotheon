...
interface ApyTrendsChartProps {
  data: Protocol[];
  showDetailed?: boolean;
}

export default function ApyTrendsChart({ data }: ApyTrendsChartProps) {
  ...
  const formatChartData = () => {
    ...
    data.forEach((protocol) => {
      // Use the first pool's APY history for simplicity
      // In a real app, you might want to calculate a weighted average
      if (protocol.pools.length > 0) {
        const mainPool = protocol.pools[0];

        mainPool.history.forEach((point) => {
          if (point.timestamp) {
            const date = point.timestamp.split("T")[0];
            if (dateProtocolMap[date]) {
              dateProtocolMap[date][protocol.name] = point.apy;
            }
          }
        });
      }
  ...
  const hasValidData =
    chartData.length > 0 &&
    data.some((protocol) =>
      chartData.some((point) => point[protocol.name] !== undefined)
    );
                  ...
                  {payload.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2 mb-1">
                      ...
                      <div className="text-sm font-medium">
                        {formatProtocolName(String(entry.name || ""))}
                      </div>
